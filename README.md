...
2, 正常编译的那台机通过传递内核Makefile的参数CC来指定编译器为kerncc，kerncc默认会将PATH中的名为gcc的文件作为编译器，设置环境变量KERNCC可以修改kerncc所使用的编译器。<br/><br/>

3, 内核的Makefile有一个名为parpare0的规则，这个规则生成一些必要的头文件，在另一台机上执行make parpare0后就可以开始接收编译任务，当.config修改后需要复制到另一台机并且重新执行这一步。（源代码修改后当然也需要，由于我平常都是会指定内核Makefile的O参数来指定单独的编译输出目录，所以直接rsync同步源码目录是很快的）。<br/>
